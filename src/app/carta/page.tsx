...
const MENU_ITEMS = [
  ...
  { id: 2, category: 'patatas', name: 'Patatas Bravas', desc: 'Salsa especial', price: 7.50 },
  { id: 3, category: 'carnes', name: 'Medallones de solomillo', desc: 'Queso cheddar', price: 12.80 },
...
function CartaContent() {
  ...
  const sendOrder = () => {
    ...
    window.open(`https://wa.me/34910712322?text=${encodeURIComponent(message)}`, '_blank');
  };
      ...
      <div className="bg-white border-b border-stone-200 sticky top-[88px] z-20 overflow-x-auto no-scrollbar shadow-sm">
        <div className="max-w-2xl mx-auto flex p-4 gap-3 min-w-max">
          {['all', 'patatas', 'carnes', 'vinos', 'cervezas'].map((cat) => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-6 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${activeCategory === cat ? 'bg-stone-900 text-white shadow-lg' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'}`}>
              {cat}
            </button>
          ))}
        </div>
        ...
        {filteredItems.map((item) => (
          <div key={item.id} className="bg-white p-6 rounded-sm shadow-sm border border-stone-100 flex justify-between items-start group hover:shadow-md transition-shadow">
            <div>
              ...
              <span className="text-[#b45309] font-bold mt-2 block">{item.price.toFixed(2)} €</span>
            </div>
            <button onClick={() => addToCart(item)} className="bg-stone-100 hover:bg-[#b45309] hover:text-white text-stone-900 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300">+</button>
          </div>
        ))}
      </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] z-40 p-4">
          <div className="max-w-2xl mx-auto">
              ...
              {cart.map((item, index) => (
                <div key={index} className="flex justify-between items-center text-sm border-b border-stone-100 pb-2 last:border-0">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => removeFromCart(index)} className="text-red-500 hover:text-red-700 hover:bg-red-50 p-1 rounded transition-colors" title="Eliminar plato">🗑️</button>
                  </div>
                </div>
              ))}
            </div>
            <div className="flex justify-between items-center pt-2 border-t border-stone-200">
              ...
              <button onClick={sendOrder} className="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-sm font-bold text-sm uppercase tracking-wider transition-colors flex items-center gap-2 shadow-lg">
...
export default function CartaPage() {
  return <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-[#fdfbf7]">Cargando...</div>}><CartaContent /></Suspense>;
}